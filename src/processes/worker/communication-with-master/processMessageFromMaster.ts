import { ActionData, ErrorData, actionsSentToMaster } from '@processes/worker/communication-with-master/sendUserMessageToMaster';
...
import { RoutingError, NonExistingEndpointRequestError, RequestMethodNotSupportedError } from '@classes/errors/routingErrors';
...
process.on('message', (message: string) => {
    ...
    if (messageFromMaster.error) {
        sendErrorToUser(getErrorClassObject(messageFromMaster.error), actionSentToMaster.res);
    } else {
        sendResponseToUser(actionSentToMaster.res, messageFromMaster.data, actionSentToMaster.reqMethod);
    }
});
